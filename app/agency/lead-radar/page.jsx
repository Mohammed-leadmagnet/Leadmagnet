...
export default function LeadRadar() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      setSubscription(sub); setCheckingAccess(false);
      if (sub && sub.plan === "scale" && (sub.status === "active" || sub.status === "trialing")) { loadClients(data.user.id); loadCredits(data.user.id); }
    });
  }, []);
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#060a07", fontFamily: "'Plus Jakarta Sans','Inter',sans-serif", color: "#d1e0d6" }}>
      ...
      : <div className="ct">
          ...
          {!selectedClientId ? <div className="sec"><div className="emp"><span className="emp-i">🛰️</span><div className="emp-t">Select a client to get started</div><div className="emp-s">Choose a client above to build their ICP, import leads, and run scoring.</div></div></div>
          : <>
            <div className="tabs">
            ...
            {activeTab === "leads" && <>
              ...
              : filteredLeads.length === 0 ? <div className="sec"><div className="emp"><span className="emp-i">👥</span><div className="emp-t">{leads.length === 0 ? "No leads yet" : "No leads match filters"}</div><div className="emp-s">{leads.length === 0 ? "Add leads manually, import CSV, sync campaign leads, or run scoring." : "Adjust your filters."}</div></div></div>
              : <div className="tbw" style={{ overflowX: "auto" }}><table className="tb" style={{ minWidth: "900px" }}><thead><tr><th>Name</th><th>Company</th><th>Title</th><th>Source</th><th>Score</th><th>Status</th><th>Actions</th></tr></thead><tbody>
                {filteredLeads.map(l => { const sc = l.lead_scores?.[0]; const b = getBadge(sc?.temperature); return (
                  <tr key={l.id}>
            ...
            {activeTab === "icp" && <div className="sec">
              ...
              <p style={{ fontSize: "0.82rem", color: "#3d5240", marginBottom: "1.5rem", lineHeight: 1.6 }}>Define the perfect lead for this client. Lead Radar uses this to score every prospect.</p>
              {icpLoading ? <div style={{ textAlign: "center", padding: "2rem", color: "#3d5240" }}>Loading...</div> : <>
                {[{ key: "target_industries", label: "Target Industries", help: "e.g. SaaS, Marketing" }, { key: "target_locations", label: "Target Locations", help: "e.g. Netherlands, USA" }, { key: "job_titles", label: "Target Job Titles", help: "e.g. CEO, Founder" }, { key: "keywords", label: "Keywords", help: "Topics that signal fit" }, { key: "competitors", label: "Competitors", help: "Companies whose customers fit" }].map(f => (<div key={f.key}><label className="fl">{f.label}</label><div className="fh">{f.help}</div><div className="tw">{icpForm[f.key].map((t, i) => <span key={i} className="tg">{t}<span className="tg-x" onClick={() => removeTag(f.key, i)}>×</span></span>)}<input className="tf" placeholder="Type and press Enter..." value={tagInputs[f.key]} onChange={e => setTagInputs(p => ({ ...p, [f.key]: e.target.value }))} onKeyDown={e => handleTagKeyDown(f.key, e)} /></div></div>))}
                ...
                <hr className="hr" />
                {[{ key: "excluded_industries", label: "Excluded Industries", help: "Industries to skip" }, { key: "excluded_titles", label: "Excluded Titles", help: "Titles to ignore" }].map(f => (<div key={f.key}><label className="fl">{f.label}</label><div className="fh">{f.help}</div><div className="tw">{icpForm[f.key].map((t, i) => <span key={i} className="tg" style={{ background: "rgba(239,68,68,0.06)", borderColor: "rgba(239,68,68,0.15)", color: "#f87171" }}>{t}<span className="tg-x" onClick={() => removeTag(f.key, i)}>×</span></span>)}<input className="tf" placeholder="Type and press Enter..." value={tagInputs[f.key]} onChange={e => setTagInputs(p => ({ ...p, [f.key]: e.target.value }))} onKeyDown={e => handleTagKeyDown(f.key, e)} /></div></div>))}
            ...
            {activeTab === "credits" && <>
              <div className="sts">
                ...
                <div className="st"><div className="st-v" style={{ color: "#22c97a" }}>{creditsLimit}</div><div className="st-l">Limit</div></div>
                <div className="st"><div className="st-v" style={{ color: "#3d5240", fontSize: "1rem" }}>{credits?.reset_date || "—"}</div><div className="st-l">Reset</div></div>
              </div>
              <div className="sec"><div className="sec-t">Credit Usage</div><div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.78rem", color: "#3d5240", marginBottom: "0.4rem" }}><span>{creditsUsed} used</span><span>{creditsRemaining} remaining</span></div><div className="cbw"><div className="cb" style={{ width: `${creditsPct}%`, background: creditsPct > 90 ? "#f87171" : creditsPct > 70 ? "#fbbf24" : "#22c97a" }} /></div><div style={{ marginTop: "1.25rem", fontSize: "0.82rem", color: "#3d5240", lineHeight: 1.6 }}>Each lead scored uses 1 credit. AI recommendations use 1 additional credit. Resets monthly.</div></div>
            </>}

            <div className="dis">Lead Radar uses approved sources, existing campaign activity, uploaded data, and public business signals. Users are responsible for complying with platform terms and privacy rules.</div>
          </>}
        </div>}
      ...
      {showAddLead && <div className="mo"><div className="md">
        ...
        <div className="fr"><div><label className="fl">Job Title</label><input className="inp" value={leadForm.title} onChange={e => setLeadForm(p => ({ ...p, title: e.target.value }))} placeholder="CEO" /></div><div><label className="fl">Company</label><input className="inp" value={leadForm.company} onChange={e => setLeadForm(p => ({ ...p, company: e.target.value }))} placeholder="Acme Corp" /></div></div>
        ...
        <div className="mb"><button className="mc" onClick={() => setShowAddLead(false)}>Cancel</button><button className="ms" onClick={handleAddLead} disabled={addingLead || (!leadForm.name && !leadForm.first_name)}>{addingLead ? "Adding..." : "Add Lead →"}</button></div>
      </div></div>}

      {/* LEAD DETAIL + AI MODAL */}
      {detailLead && <div className="mo"><div className="md">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.25rem" }}>
          <div>
            <div className="md-t" style={{ marginBottom: "0.25rem" }}>{detailLead.name || "Unknown"}</div>
            {detailLead.title && <div style={{ fontSize: "0.84rem", color: "#4d6b54" }}>{detailLead.title}{detailLead.company ? ` at ${detailLead.company}` : ""}</div>}
          </div>
        ...
        <div className="sec" style={{ marginBottom: "0.75rem" }}>
          <div className="sec-t">Lead Details</div>
          {[["Company", detailLead.company], ["Industry", detailLead.industry], ["Location", detailLead.location], ["Email", detailLead.email], ["LinkedIn", detailLead.linkedin_url], ["Source", detailLead.source_type?.replace(/_/g, " ")], ["Status", detailLead.status]].filter(([, v]) => v).map(([k, v]) => <div className="d-row" key={k}><span className="d-key">{k}</span><span className="d-val">{v}</span></div>)}
        </div>
        ...
        {detailLead.lead_scores?.[0] && (() => { const sc = detailLead.lead_scores[0]; const b = getBadge(sc.temperature); return (
          <div className="sec" style={{ marginBottom: "0.75rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
              <div className="sec-t" style={{ marginBottom: 0 }}>Score Breakdown</div>
              <span className="sb" style={{ background: b.bg, border: `1px solid ${b.border}`, color: b.color, fontSize: "0.82rem", padding: "0.3rem 0.75rem" }}>{sc.total_score}/100 · {b.label}</span>
            </div>
            {[{ name: "Fit", val: sc.fit_score, color: "#22c97a" }, { name: "Intent", val: sc.intent_score, color: "#a78bfa" }, { name: "Contactability", val: sc.contactability_score, color: "#63b3ed" }, { name: "Freshness", val: sc.freshness_score, color: "#fbbf24" }].map(s => (
              <div className="sc-bar-wrap" key={s.name}><div className="sc-bar-label"><span className="sc-bar-name">{s.name} ({s.name === "Fit" ? "40%" : s.name === "Intent" ? "30%" : s.name === "Contactability" ? "20%" : "10%"})</span><span className="sc-bar-val" style={{ color: s.color }}>{s.val}</span></div><div className="sc-bar"><div className="sc-fill" style={{ width: `${s.val}%`, background: s.color }} /></div></div>
            ))}
            {sc.recommended_action && <div style={{ marginTop: "0.75rem", fontSize: "0.82rem", color: "#8fa696", fontFamily: "Inter,sans-serif" }}>💡 {sc.recommended_action}</div>}
          </div>
        ); })()}

        {/* AI Recommendation */}
        <div className="sec" style={{ marginBottom: "0.75rem" }}>
          <div className="sec-t">🤖 AI Recommendation</div>
          {aiRec ? <>
            <div className="ai-box"><div className="ai-label">Why Valuable</div><div className="ai-text">{aiRec.why_valuable}</div></div>
            <div className="ai-box"><div className="ai-label">Outreach Angle</div><div className="ai-text">{aiRec.outreach_angle}</div></div>
            <div className="ai-box"><div className="ai-label">Suggested Message</div><div className="ai-text">{aiRec.suggested_message}</div></div>
            <div className="ai-box"><div className="ai-label">Risk Warning</div><div className="ai-text">{aiRec.risk_warning}</div></div>
            <div className="ai-box"><div className="ai-label">Next Action</div><div className="ai-text">{aiRec.next_action}</div></div>
          </> : <div style={{ textAlign: "center", padding: "1rem" }}>
            <button className="btn btn-p" onClick={() => getAiRec(detailLead)} disabled={aiLoading} style={{ fontSize: "0.84rem", padding: "0.65rem 1.25rem" }}>{aiLoading ? "Generating..." : "🤖 Get AI Recommendation (1 credit)"}</button>
            <div style={{ fontSize: "0.72rem", color: "#2a3d2e", marginTop: "0.5rem" }}>Uses GPT-4o to analyze this lead and suggest outreach strategy</div>
          </div>}
        </div>

        {/* Actions */}
        <div style={{ display: "flex", gap: "0.5rem" }}>
          {detailLead.status !== "approved" && <button className="btn btn-s" onClick={() => updateStatus(detailLead.id, "approved")}>✓ Approve</button>}
